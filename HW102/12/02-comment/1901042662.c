...
int main(){
    ...
    printf("User Name: "); /*taking username*/
    get_string(stdin,&user_name);

    strcpy(user_fname,user_name);
    strcat(user_fname,".worddat"); /*adding .worddat to the end of the username */
    
    check = login(user_fname); /*checks if such a user exists*/
    ...
    if(check){
        update_from_userdata(user_fname,list_an,list_sn);/*reads user data and updates lists if a registered user is logged in*/
...
int login(char *str){ /*checks if such a user exists, returns 1 if there is, otherwise creates a new user and returns 0*/
...
/*-------------------------------------------------------*/
void update_from_userdata(char * user_fname,antonyms *list_an,snonyms *list_sn){ /*reads user data and updates lists*/
